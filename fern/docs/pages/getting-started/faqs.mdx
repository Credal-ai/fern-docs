...
**Why is it taking my agent longer to respond to my query than when I ask chat GPT, Gemini, or another model directly?**

Credal agents are often a little slower than a straight‑through ChatGPT or other LLMs because they are doing different, more extensive, work. For example, Credal agents may be searching your enterprise's non-public internal document collections or orchestrating with subagents, which takes additional milliseconds to seconds. You can speed up agents' responses in a number of ways:
...
3\. Try changing the model (for instance to Gemini Flash)

4\. Avoid using sub-agents with very simple agents who don't need subagents.

ChatGPT and other LLMs, used in isolation, are searching exclusively public sources and are not orchestrating anything at all - and are thus often extremely speedy. But do not confuse a speedy response for a high-quality one.
...
Credal supports [_out-of-the-box integrations_](https://www.credal.ai/security) with major enterprise tools, including:
