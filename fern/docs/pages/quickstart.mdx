...
Agents are purpose-built AIs you create by selecting a model, attaching data sources, and writing a custom prompt. You can interact with them through a chat UI or deploy them to Slack and other applications to perform workflows or answer questions.
...
4. **Prompt** – Here you can provide a custom prompt that gives the agent context and instructions on how to handle user queries. By default, Credal instructs agents to be helpful, honest, and direct, and to let users know when they're unsure of the answer.
...
- **All relevant messages**: will respond to all queries matching the [Description](/platform/agents/configure-an-agent/a-name-and-description) of the agent.
...
📘 Looking for a more in-depth guide? Refer to the [Agents Platform section](https://docs.credal.ai/platform/agents/setting-up-an-agent)
