...
4. Enable [Code Interpreter](/user-guide/platform/agent-builder/configuration/choosing-a-model) (required for image generation)
...
2. **Action Access**:
   - You can specify exactly which users or groups can use this Action through the [End User Access settings](/user-guide/platform/governed-actions/credentials-and-security)
...
It's recommended to use a [thinking model](/user-guide/platform/agent-builder/configuration/choosing-a-model) with reasoning enabled for complex queries, however the model you select also depends on the query approach. You may want to use a smaller non-reasoning model for query templates which are more well defined compared to snowflake views.
...
Read about each approach and implementation steps in [Query Approaches](/user-guide/getting-started/tutorials-common-first-agents/snowflake-query-assistant/query-approaches).

## Next Step

Continue to [Query Approaches](/user-guide/getting-started/tutorials-common-first-agents/snowflake-query-assistant/query-approaches) to compare query templates versus Snowflake views and choose the implementation pattern that fits your use case.
