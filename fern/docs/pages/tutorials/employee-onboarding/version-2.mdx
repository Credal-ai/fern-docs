...
- Configure appropriate search tools (deep think/[smart filtering](/user-guide/platform/agent-builder/configuration/add-connectors/document-collections/smart-filtering-beta))
...
Attach both the Company Information Domain Specialist and Web Searcher as subagents via [Agent Collaboration](/user-guide/platform/agent-builder/configuration/agent-collaboration) to The Orienteer agent. Update the instructions prompt to:
...
Continue to [Connect with Internal Systems](/user-guide/getting-started/tutorials-common-first-agents/employee-onboarding-assistant/connect-with-internal-systems) to add feedback-driven actions like Jira ticket creation and FAQ updates.
