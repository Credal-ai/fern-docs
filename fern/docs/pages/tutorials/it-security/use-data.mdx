...
Let's first set up our agent with context about the IT department so it can answer user questions successfully. If you navigate to the [Data Sources](/user-guide/platform/agent-builder/configuration/add-connectors/data) section, you can attach sources from Google Drive, Notion, Slack, etc. Anywhere you have useful information about IT is fair game.

![Data sources](/docs/assets/it-security-use-cases/it-sources.png)

![Data sources](/docs/assets/it-security-use-cases/it-sources-2.png)

Once data is attached to the agent, we will want to supply it with a [detailed prompt](/user-guide/platform/agent-builder/configuration/writing-instructions) about how to use that data and conversationally interface with users when answering questions. We can use the following example prompt:
...
Continue to [Track Tickets in Jira](/user-guide/getting-started/tutorials-common-first-agents/it-security-assistant/track-tickets-in-jira) to add ticket creation and status management to the IT assistant.
