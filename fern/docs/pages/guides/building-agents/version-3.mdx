...
5. **Respond to the user with what happened (and hopefully an answer!) while soliciting feedback about things that were unclear for the agent builder and hiring manager to make the process smoother.** If the answer is incomplete, tag the relevant SME.
6. **If the feedback is actionable (requires a bug fix, no answer was provided, or any other human intervention), create a Jira ticket in the ONB Credal Project**
7. **If the feedback is positive and the answer was helpful and correct, initiate the process to update the FAQ document with the Q/A pair**
...
### Step 1: Modifying the prompt to solicit feedback
...
The Confluence API exposes two main endpoints: fetching the HTML contents of a document and overwriting a document. Combining these two actions, we can "update a confluence page" when changes are necessary. Let's create those Actions and attach them to our Agent. Please see the Actions documentation to learn how to do this!
...
Ask a Credal admin to publish this Agent to your "Consult Employee Onboarding FAQs" Agent and attach it as an Action.
