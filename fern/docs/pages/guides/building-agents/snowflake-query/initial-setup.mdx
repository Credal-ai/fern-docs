...
4. **Role**: The role you would like to use with the credentials (if not set it will use the default role in snowflake for the credential)
...
### Step 3: Attach your snowflake credential to your Snowflake Action 
- Access to the snowflake credential is controlled by who has access to your action.
- You can control who can run use this action by setting end-user-access on the actions it's attached to.
- Only the creator of a snowflake credential can add it to a snowflake action 
<Frame>
