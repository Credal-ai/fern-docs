...
- **When and How**: Any timing constraints or procedural guidelines.
...
Providing a clear template for expected responses is crucial. Here’s an example of setting up an output format for entity extraction:
...
Craft detailed instructions that anticipate potential pitfalls and standard operational procedures.
...
-Include all review tags in the table and each review tag (e.g., "[T] Disclosing private OpenSea/TaskUs Information e.g. leaking product launches") should be its own row with corresponding feedback
...
Opt for straightforward language to prevent ambiguity:

**Less effective:**

```
Could you elucidate the benefits associated with utilizing Credal AI?
...
```
What are the benefits of using Credal AI?

```

## 7. Save your work in a “Suggested Question”
...
![prompt-engineering.png](/docs/assets/bulk-analysis/in-depth-guide/prompt-engineering.png)
...
Continuously refine your prompts based on the responses received to improve accuracy and relevance.

We are currently working on some changes that will incorporate negative feedback into the copilot config workflow by suggesting ways in which you can improve your background prompt.

## 10. What not to ask…

1. Anything about access controls. That’s on us, and your prompt about access won’t make sense to the LLM provider we send your prompt to!
