...
- The “write” Actions are typically void and don’t bring in sensitive information into the chat history, but the “read” Actions pose DLP risks. Note: We do not recommend expanding access permissions in Agents directly available to end users, rather you should use Agent<>Agent handoffs for this (see below). Nonetheless, the below features and considerations make multi-user requests airtight in Credal.
...
|------------------------|---------------------------------------------------------------------|------------------------------------------------------------------------------------------|-------------------------------------------|----------------------------------------|
| Non exhaustive Source examples | Salesforce / Credal enterprise search / Firecrawl web search / Jamf / Kanji | Slack / MongoDB / Gong | Looker / Snowflake / (Formerly Kanji & Jamf) | Webhook |
| Where is resource level access handled? | In the provider’s implementation | In Credal’s implementation of interfacing with provider API | Define trusted allowlist at provider level and Credal limits configurability within the action instead of relying on admin judgement. - scrapping this, | Trusted allowlist defined in action, If HITL enabled, it’s set of approvers, else it is set of end users with access - trust the admin publishing to verify this is correct |
| Create the connection? |  |  |  |  |
| Create an Action? |  |  |  | |
| Edit the Action? |  |  |  Until we have version control for Actions, this will require an admin user for changes |  Until we have version control for Actions, this will require an admin user for changes |
| Publish the Action to the whole organization? |  |  |  Approvers are set to trusted set of approvers OR no approvers and end user access is set to trusted set of approvers. (applies for all of the below) |  Approvers are set to trusted set of approvers OR no approvers and end user access is set to trusted set of approvers. (applies for all of the below) |
| Publish the Action to Select Users/Groups? |  |  |  |  |
| Publish the Action to All Agents in Organization |  Not for security purposes |  Not for security purposes |  |  |
| Publish the Action to Selected Agents |  |  |  |  |
| Invoke the Action? |  |  |  |  |