...
_It’s helpful to have a good sense of what you want your copilot to do from the start, so you can set it up to achieve its objectives._

To get started:

1. On your left sidebar, Select “Copilots”:

![copilots_lhs](/docs/assets/copilots/copilots_lhs.png)

1. Select the “Create new copilot” button.

![create_new_copilot_button](/docs/assets/copilots/create_new_copilot_button.png)
...
![model_selection.png](/docs/assets/copilots/model_selection.png)

GPT-4o (the default) is a good starting point for most use cases, as it’s very capable and relatively affordable. If you are unsure of the best choice, running some test queries is a good way to gauge a model’s performance for your use case. Gemini has a larger context window and can be helpful to handle longer inputs and detailed queries.
...
Here you can provide a background prompt, which will provide the copilot with context and instructions on how to handle user queries. By default, Credal instructs copilots to be helpful, honest, and to the point and to let users know when it is unsure of the answer. Check out our [Prompting guide] (https://docs.credal.ai/getting-started/prompting) for more details. 

![background_prompt.png](/docs/assets/copilots/background_prompt.png)
...
_You are a diligent, objective, detail-oriented, assistant. Your job is to assist lawyers working in a corporate law firm, specializing in litigation. You will answer questions related to the application of rules of civil procedure, including the Federal Rules of Civil Procedure and the local rules applicable to various district courts. You will respond to questions and prompts truthfully._
...
- _When providing a response, always include a reference to the rule number which contains the answer, using the format “1.A.iii”_
...
](/docs/assets/copilots/copilot_suggested_questions.png)
...
![saved_suggested_questions_display.png](/docs/assets/copilots/saved_suggested_questions_display.png)
...
![Here, we pinned our infosec FAQs when setting up Credal’s Infosec copilot. ](/docs/assets/copilots/pinned_infosec_FAQs_example.png)

Here, we pinned our infosec FAQs when setting up Credal’s Infosec copilot.

**Pinned Data example use case**: When generating content such as a case study or a newsletter, pinned data can be precedent work that is used as reference for formatting, comprehensiveness, tone, and specificity. 

1. **Searchable Sources**

You should also provide your copilot with sufficient data relevant to its area of expertise. Searchable Sources is typically a larger set of data, such as numerous help desk articles and a collection of websites, that may not in its entirety be relevant to the particular query. Perhaps only some articles, and further, only a subset of these articles would be helpful. The copilot will search these sources for information relevant to user queries. Use the toggle to enable searchable sources and use the search bar to add sources.

![We provided our infosec copilot with all of Credal’s information security documentation. ](/docs/assets/copilots/infosec_copilot_searchable_data_example.png)
...
<aside>
💡 Clearly naming your documents can help your copilot distinguish and identify them. This can be helpful if you want to ask questions that refer to specific documents.
...
![copilot_user_inputs_section.png](/docs/assets/copilots/copilot_user_inputs_section.png)

### f. Tools

Copilots have the ability to use predefined tools. Many of the tools are in Beta, so reach out to [support@credal.ai](mailto:support@credal.ai) if you have any questions or feedback.

![Copilot Tools.png](/docs/assets/copilots/Copilot_Tools.png)

1. **Code Interpreter**
   1. Code Interpreter is primarily used to generate charts and graphs. Your copilot will use the code interpreter tool to write a Python script, and will use that Python script to generate a chart or graph. The chart will be displayed visually in the message response.
...
2. **Smart Filtering**
   ...
   1. Your copilot can use the Smart Filtering tool to automatically figure out exactly what data is relevant to the user’s question.
   2. The copilot will generate filters based on the user’s question, then apply those filters to restrict what data it considers. This is what it looks like in action:
   ...
   1. First, you must have a Document Collection attached in the Data section of the copilot under “Search”.
...
iii. **Deep Thinking Mode**
   ...
   1. Deep Thinking allows the copilot to have a more comprehensive search about key topics across your data and then combines the results, ouputting a higher quality synthesis.
...
1. Web Search lets your copilot search the web for relevant sources, then use those sources to generate an answer.
2. Behind the scenes, the copilot will use Bing to search the web based on the user’s question, then use up to five results.
...
![enable_copilot_in_credal_ui](/docs/assets/copilots/enable_copilot_in_credal_ui.png)

The copilot will now be available for everyone at your organization to chat with:

![chat_with_copilot.png](/docs/assets/copilots/chat_with_copilot.png)
...
You can connect your copilot to other applications or build your own tools that route queries to your copilot through a unique API. Use the toggle to enable API access and generate a copilot API key. See our [API documentation](https://docs.credal.ai/api-reference/copilots) for more information on implementation.

![enable_copilot_api_access.png](/docs/assets/copilots/enable_copilot_api_access.png)

### c. Custom Slack Apps (Branding, Name, Profile Picture)

Credal supports custom Slack Apps, commonly known as Slack "bots", for your individual Copilot.
...
![copilot_chat_interface_example.png](/docs/assets/copilots/copilot_chat_interface_example.png)
...
![*Here, the Copilot responded as the question fell within the its scope—questions on information security at Credal.*](/docs/assets/copilots/slack_relevant_query_response.png)

_Here, the Copilot responded as the question fell within the its scope—questions on information security at Credal._

![
*This question fell outside of the Copilot’s scope, and it did not respond.*](/docs/assets/copilots/slack_ignored_query_example.png)

_This question fell outside of the Copilot’s scope, and it did not respond._
...
![share_copilot_option.png](/docs/assets/copilots/share_copilot_option.png)
