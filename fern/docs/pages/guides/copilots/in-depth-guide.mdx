_This is a comprehensive in depth guide for Agents. If you’re just getting started, check out [Getting Started With AI Agents](/getting-started/agents/quick-start)._
...
![agents_lhs](/docs/assets/copilots/copilots_lhs.png)

1. Select the “Create new agent” button.

![create_new_agent_button](/docs/assets/copilots/create_new_copilot_button.png)

1. Give your agent a name and add a description of its function in the pop-out form. You can always come back and edit these later.

![agent_description](/docs/assets/copilots/copilot_description.png)

1. Then select “Create Agent.”
...
More advanced models, like o1, are both more expensive and more capable than earlier models. When selecting a model, you may want to consider your budget, the volume of queries you expect the agent to receive, and the complexity of the task.
...
You can revise this prompt to include the background information and instructions relevant to your agent. When thinking about the level of detail you should provide, consider how you might explain the agent’s assignment to a new employee. Your explanation might include:
...
### **Example Prompt 1: Federal Rules of Court Agent**
...
### **Example Prompt 2: Credal’s Information Security Agent**
...
By pinning data sources, you can instruct your agent to refer to certain sources when answering _every_ user query. You should use this for documents that will relate to most queries you expect your agent to address. If you have a document that you almost always want to agent to refer to, put it here. This could include answers to FAQs or a sales playbook. As pinned sources will be read in their entirety every time a user asks a question, they should only be used for a limited amount of high quality data to avoid overwhelming the AI with too much data on every question.
...
- **All messages matching filter**: will respond to messages that match the filter criteria. In the example on the left, the agent will only respond to prompts containing “?”.
...
You can connect your agent to other applications or build your own tools that route queries to your agent through a unique API. Use the toggle to enable API access and generate a agent API key. See our [API documentation](https://docs.credal.ai/api-reference/agents) for more information on implementation.
