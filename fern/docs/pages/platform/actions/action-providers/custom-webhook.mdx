...
<video src="/docs/assets/actions/custom_webhook.mp4" width="100%" controls></video>

### (Optional) Include X-Credal-User Header

If you check the **Include X-Credal-User header** box (visible under the spec input), the outbound request will include an `X-Credal-User` HTTP header whose value is the authenticated Credal user invoking the action (e.g. their email or user id). This is useful for:
