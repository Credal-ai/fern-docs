...
An operation is a multi-user operation if the approval to carry out a given action in the workflow comes from someone other than the sender. Credal has human-in-the-loop (HITL) approval mechanisms that you can configure to use your credentials or permission scopes in a cross-functional or multi-user operation. These mechanisms build on the [action security model](/user-guide/platform/governed-actions/credentials-and-security). A request that is not initiated by you can only use your credentials/scopes if you give explicit permission to do so.
...
- The guardrails that already exist in the underlying systems remain the ultimate source of truth; Credal just adds a programmable, fully-logged wrapper on top. Credal never manufactures new privileges out of thin air and is simply automating the same approval you would have done manually (like a ping in Slack or a tap on the shoulder)—except now with a perfect audit trail.
- For these workflows, the approver—not the requester—is responsible in the audit trail.
...
- The "write" actions are typically void and don't bring sensitive information into the chat history, but the "read" actions pose data loss prevention (DLP) risks. Note: We do not recommend expanding access permissions in agents directly available to end users; rather, you should use agent-to-agent handoffs for this (see below). Nonetheless, the features and considerations below make multi-user requests airtight in Credal.
- Let’s say you need to fetch specific sensitive data from a database to answer an employee’s yes/no/etc. question, but they are not allowed to see the raw underlying data. There are a few risks associated with permissions enhancement when automating these sorts of Enterprise operations.
  ...
  - Adversarial prompt injection (user asking for something they shouldn’t be)
    - For fields associated with user identity, Credal will assign [caller-set parameters](/user-guide/platform/governed-actions/action-parameters) to hard code the sender’s email, user ID, requesting agent ID, etc. This will guarantee that Bob can’t ask for Mary’s salary information, for example.
...
We're actively developing additional human approval capabilities. If you'd like early access to any of these features, please [contact Credal](https://credal.ai/contact):
