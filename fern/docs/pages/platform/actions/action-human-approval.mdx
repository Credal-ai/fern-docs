...
An operation is a multi-user operation if the approval to carry out a given action in the workflow comes from someone other than the sender. Credal has human-in-the-loop (HITL) approval mechanisms that you can configure to use your credentials or permission scopes in a cross-functional or multi-user operation. A request that is not initiated by you can only use your credentials/scopes if you give explicit permission to do so.
