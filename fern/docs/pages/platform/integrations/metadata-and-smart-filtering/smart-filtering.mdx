...
We support smart filtering for all of our data sources. This allows you to filter on fields that you may have imported (from semi-structured data sources like Salesforce and Jira) or metadata you have tagged documents with manually using the data catalog [metadata API endpoint](/api-reference/v-0/document-catalog/metadata), the [document upload API](/api-reference/v-0/document-catalog/upload-document-contents), or even automatically extracted metadata using AI extracted entities [see this section](/user-guide/platform/integrations/metadata-and-smart-filtering/ai-entity-extraction-beta). This allows us to only send relevant information to the LLM.
