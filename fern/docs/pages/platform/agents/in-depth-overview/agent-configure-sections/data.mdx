...
Searchable Sources is typically a larger set of data, such as numerous help desk articles and a collection of websites, that may not in its entirety be relevant to the particular query. Perhaps only some articles, and further, only a subset of these articles would be helpful. The agent will search these sources for information relevant to user queries.
