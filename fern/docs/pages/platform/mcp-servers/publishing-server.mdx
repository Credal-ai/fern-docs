...
To make Credal MCP servers available in Claude or ChatGPT, an admin needs to complete the organization-wide setup in [Connect MCP Clients](/user-guide/platform/administration/connect-mcp-clients) and connect the following URLs once at the organization level:
...
Each server also exposes an advanced endpoint that unlocks features like human approval for tool calls. See [Advanced Usage](/user-guide/platform/mcp-server-builder/advanced-usage) for details.
