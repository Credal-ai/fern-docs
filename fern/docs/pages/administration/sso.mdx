...
Enabling group push via SAML/SCIM allows administrators to manage Credal role assignments from their identity provider, including mapping groups to roles such as Limited, Standard, Agent Creator, Developer, and Admin.

You can review the current role matrix in [Access Control Settings](https://app.credal.ai/admin/access-controls#roles).

### Benefits of SAML/SCIM

- **Automated User Provisioning** - Automatically create user accounts when added to your identity provider
- **Real-time Deprovisioning** - Immediately revoke access when users leave your organization (all actions are captured in the [Audit Log](/user-guide/platform/administration/data-warehouse-integration))
...
To configure SAML/SCIM for your Credal deployment, please contact your dedicated Credal team. Our team will send you an automated setup tutorial to:
