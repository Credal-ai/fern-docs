...
| `documentExternalId` | Yes | Your stable identifier for the document. Uploads with the same ID update the existing document. |
| `documentName` | No | Display name. Defaults to the uploaded filename. |
| `collectionId` | No | UUID of the collection to add the document to. |
| `documentExternalUrl` | No | Link to the document in your source system. |
| `allowedUsersEmailAddresses` | No | Emails of users allowed to see the document, as a JSON array or comma-separated list. |
| `customMetadata` | No | JSON string containing custom metadata. |
| `forceUpdate` | No | Set to `"true"` to update even if the contents are unchanged. |
